...
const doSmooth = true;
const smoothAmount = 0.9;
const smoothTrimTolerance = 7;
...
Sequence.fromStatement("repeat 32,0,48,0,64,0 AS YY", 288);
  ...
  if (doSmooth) {
    paths.forEach((path) => {
      ...
      if (!doSpiral) {
        ...
        let iter = 0;
        while (
          !GeomHelpers.pointsAreEqual(lastPt, pts[0], smoothTrimTolerance)
        ) {
          pts.shift();
          iter++;
          if (iter > 100) {
