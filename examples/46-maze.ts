...
// );
Sequence.fromStatement("shuffle 0x969999,0x979999,0x989999,0x999999 AS COL");
  ...
  for (let yy = 0; yy < ny; yy++) {
    for (let xx = 0; xx < nx; xx++) {
      ...
      shapes.forEach((shape) => {
        ...
        let colorIdx = cell ? (cell.values[5] - 1) % 4 : 1;

        shape.style = {
          fillAlpha: 0,
          fillColor: allColors[colorIdx],
          strokeColor: allColors[colorIdx],
          strokeThickness: 1,
          hatchStrokeColor: allColors[colorIdx],
