import { IDistributeHandler, IGridDistributeParams, IGrid2DistributeParams } from "./distribute-interfaces";
...
export class GridDistributeHandler2 implements IDistributeHandler {

  private _resolvedParams: IGrid2DistributeParams;
  private _positions: { x: number; y: number }[] = [];

  constructor(params: IGrid2DistributeParams) {
    this._resolvedParams = {
      ...params,
      columns: $(params?.columns) || 1,
      rows: $(params?.rows) || 1,
  ...
  arrangeShapes(shapes: IShape[], params: IShapeParams, context: IShapeContext): void {
    ...
    shapes.forEach((shape, index) => {
      if (index < this._positions.length) {
        ...
        const offsetX = $(params.offsetX || 0);
        const offset = new Point(
          (this._resolvedParams as IGrid2DistributeParams).negateOffsetX ? -offsetX : offsetX,
          $(params.offsetY || 0),
        );
        GeomHelpers.rotatePoint(offset, Math.PI - context.getCursorDirection());
