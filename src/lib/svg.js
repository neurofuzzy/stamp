...
const TextRenderer = require("./lib/TextRenderer");
...
class SVG {
  ...
  static shapesToPaths(
    ...
    shapes.forEach((shape) => {
      ...
      if (shape.data && shape.data.color) {
        stroke = isNaN(shape.data.color)
          ? shape.data.color
          : "#" + shape.data.color.toString(16);
      ...
      while (lastSegIdx < segs.length) {
        let pathStr = shapePathMarkup.replace(
          "{{fill}}",
          shape.isOpen ? `fill="none"` : `fill="${fill}"`,
        );
        ...
        for (i = lastSegIdx; i < segs.length; i++) {
          ...
          if (
            i > lastSegIdx &&
            GeomUtil.segmentsConnected(segs[i - 1], segs[i])
          ) {
            command = "L";
          } else {
  ...
  static linesToPaths(
    ...
    if (segs.length) {
      if (segs[0].data && segs[0].data.color) {
        stroke = isNaN(segs[0].data.color)
          ? segs[0].data.color
          : "#" + segs[0].data.color.toString(16);
        console.log("color assigned", stroke);
    ...
    if (segs && segs.length) {
      for (let i = 0; i < segs.length; i++) {
        ...
        if (
          i == 0 ||
          (!SVG.noPenUp &&
            !GeomUtil.pointsEqual(segs[i - 1].b, segs[i].a, SVG.equalScale))
        ) {
          if (SVG.drawLinesBackAndForth && cache.length) {
            cache = cache.reverse();
            cache[0] = "L" + cache[0].substr(1);
            pointsStr += cache.join("");
          }
          ps = `M ${lop((segs[i].a.x - bb.minX) * scale + offsetX + margin)} ${lop((segs[i].a.y - bb.minY) * scale + offsetY + margin)} `;
          cache = [
            `L ${lop((segs[i].a.x - bb.minX) * scale + offsetX + margin)} ${lop((segs[i].a.y - bb.minY) * scale + offsetY + margin)} `,
          ];
          pointsStr += ps;
        }
  ...
  static curvesToPaths(
    ...
    curves.forEach((curve) => {
      ...
      if (curve.data && curve.data.color) {
        stroke = isNaN(curve.data.color)
          ? curve.data.color
          : "#" + curve.data.color.toString(16);
      }
  ...
  static mixedToSVG(
    ...
    const bb = GeomUtil.boundingBoxesBoundingBox(
      mixed.map((elem) => elem && elem.getBoundingBox()),
    );
    ...
    if (SVG.documentSize.w > 0 && SVG.documentSize.h > 0) {
      const docBB = new BoundingBox(
        0,
        0,
        SVG.documentSize.w * 96 - margin * 2,
        SVG.documentSize.h * 96 - margin * 2,
      );
    ...
    for (let n = 0; n < nonCurves.length; n++) {
      ...
      if (
        forceToShapes ||
        (elem instanceof Shape && !elem.isOpen) ||
        elem.data.outline
      ) {
        shapes.push(elem);
      } else if (
        forceGrouped ||
        (elem instanceof SegmentCollection && elem.isGroup)
      ) {
    ...
    if (SVG.drawSkirt && SVG.documentSize.w > 0 && SVG.documentSize.h > 0) {
      ...
      out = out
        .split("<!--skirt-->")
        .join(
          `<path d="M ${w - 80} 8 L ${w - 80} 10 L ${w} 10 L ${w} 90 L ${w + 2} 90"  fill="none" stroke="red" stroke-width="2" />`,
        );
    }

    if (SVG.title && SVG.documentSize.w > 0 && SVG.documentSize.h > 0) {
      let x = SVG.documentSize.w * 96 - SVG.titleMargin - 10;
      let y = SVG.documentSize.h * 96 - SVG.titleMargin;
      let alignPoint = { x, y };
      let title = TextRenderer.render(
        SVG.title,
        SVG.documentSize.w > 5 ? 0.2 : 0.15,
        alignPoint,
        true,
        true,
      );
      let titleBb = GeomUtil.segmentsBoundingBox(title._segments);
      let mark = new Segments([
        new Segment(
          new Point(alignPoint.x + 10, alignPoint.y),
          new Point(alignPoint.x + 10, alignPoint.y - titleBb.height()),
        ),
        new Segment(
          new Point(alignPoint.x + 10, alignPoint.y - titleBb.height()),
          new Point(alignPoint.x + 10, alignPoint.y),
      ...
      let titlePaths = this.linesToPaths(
        [mark, title],
      ...
      out = out.split("<!--title-->").join(titlePaths);
    }
