...
class SVG {
  ...
  static mixedToSVG(margin, mixed, forceGrouped = false, forceToShapes = false, groupColor = null) {
    ...
    if (SVG.title && SVG.documentSize.w > 0 && SVG.documentSize.h > 0) {
      ...
      let alignPoint = { x, y };
      let title = TextRenderer.render(SVG.title, SVG.documentSize.w > 5 ? 0.2 : 0.15, alignPoint, true, true);
      let titleBb = GeomUtil.segmentsBoundingBox(title._segments);
      let mark = new Segments([
        new Segment(new Point(alignPoint.x + 10, alignPoint.y), new Point(alignPoint.x + 10, alignPoint.y - titleBb.height())),
        new Segment(new Point(alignPoint.x + 10, alignPoint.y - titleBb.height()), new Point(alignPoint.x + 10, alignPoint.y)),
      ]);
      let titlePaths = this.linesToPaths([mark, title], new BoundingBox(0, 0, 0, 0), 0, 1, 0, 0, 1);
