...
import { FlowTokenizer, TokenType } from './flow-tokenizer';
...
export class FlowRenderer {
  ...
  private readonly reservedWordTypes = ['triggers', 'conditions', 'actions', 'contexts', 'delays', 'flowcontrol'];
  
  constructor(private tokenizer: FlowTokenizer) {}

  /**
   * Render flow text into colored HTML spans (port of onCodeChanged)
   */
  render(rawText: string): RenderResult {
    if (!rawText) {
  ...
  private joinTokensWithSpaces(tokens: string[]): string {
  ...
  private getContextForWord(word: string, context: number, targetType?: number): number {
  ...
  private getTargetTypeForContext(startContext: number, subContext: number): number {
    ...
    if (startContext === 1) return 1; // conditions typically use numbers
    if (startContext === 2) return 2; // actions typically use states
    return 0;
  }
